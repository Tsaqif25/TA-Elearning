...
<div class="flex flex-col w-full bg-[#FAFAFA] font-poppins min-h-screen">
  <div class="max-w-[1200px] w-full mx-auto px-5 sm:px-6 lg:px-10 mt-8 mb-16">
    ...
    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-8 flex flex-col lg:flex-row gap-6">
      ...
      <!-- Kolom Kiri (Form) -->
      <div class="flex-1">
        <form id="formRepositoryEdit" action="{{ route('repository.update', $repository->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
          @csrf
          @method('PUT')
          ...
          <div>
            ...
            <input type="text" id="judul" name="judul" value="{{ old('judul', $repository->judul) }}" 
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select name="kelas" class="w-full border border-gray-200 rounded-xl p-3 bg-[#F9FAFB] focus:border-[#2B82FE] focus:ring-2 focus:ring-[#2B82FE]/20">
                <option value="">Pilih Kelas</option>
                <option value="10" {{ old('kelas', $repository->kelas) == '10' ? 'selected' : '' }}>Kelas 10</option>
                <option value="11" {{ old('kelas', $repository->kelas) == '11' ? 'selected' : '' }}>Kelas 11</option>
                <option value="12" {{ old('kelas', $repository->kelas) == '12' ? 'selected' : '' }}>Kelas 12</option>
              </select>
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-800 mb-2">Jurusan</label>
              <select name="jurusan" class="w-full border border-gray-200 rounded-xl p-3 bg-[#F9FAFB] focus:border-[#2B82FE] focus:ring-2 focus:ring-[#2B82FE]/20">
                <option value="">Pilih Jurusan</option>
                <option value="RPL" {{ old('jurusan', $repository->jurusan) == 'RPL' ? 'selected' : '' }}>RPL</option>
                <option value="TKJ" {{ old('jurusan', $repository->jurusan) == 'TKJ' ? 'selected' : '' }}>TKJ</option>
                <option value="Multimedia" {{ old('jurusan', $repository->jurusan) == 'Multimedia' ? 'selected' : '' }}>Multimedia</option>
                <option value="Akuntansi" {{ old('jurusan', $repository->jurusan) == 'Akuntansi' ? 'selected' : '' }}>Akuntansi</option>
                <option value="Pemasaran" {{ old('jurusan', $repository->jurusan) == 'Pemasaran' ? 'selected' : '' }}>Pemasaran</option>
              </select>
            </div>
          </div>
        ...
        <div class="divide-y divide-gray-100">
          ...
          <div class="flex items-center justify-between py-3">
            <div class="flex items-center gap-3 min-w-0">
              @php
                $icon = 'fa-file';
                if (Str::endsWith($file->file, ['.pdf'])) $icon = 'fa-file-pdf';
                ...
                elseif (Str::endsWith($file->file, ['.jpg', '.jpeg', '.png'])) $icon = 'fa-image';
                elseif (Str::endsWith($file->file, ['.mp4'])) $icon = 'fa-video';
              @endphp
      ...
      <form action="{{ route('repository.destroyFile', $repository->id) }}" method="POST" onsubmit="event.preventDefault(); handleDeleteRepo(this);" class="ml-4 flex-shrink-0">
    @csrf
    @method('DELETE')
...
function handleDeleteRepo(form) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      form.submit();
      Swal.fire({ title: 'Terhapus!', text: 'File berhasil dihapus.', icon: 'success', timer: 1500, showConfirmButton: false });
    }
  });
}
  ...
  $.ajax({
    ...
    success: function () {
      if (myDropzone.getQueuedFiles().length === 0) {
        ...
        setTimeout(() => window.location.href = "{{ route('repository.index') }}", 1500);
      } else {
        myDropzone.processQueue();
      }
    },
    error: function () {
      Toastify({ text: "Terjadi kesalahan saat menyimpan perubahan.", duration: 3000, gravity: "top", position: "right", style: { background: "#dc2626" } }).showToast();
    }
  });
});
</script>
